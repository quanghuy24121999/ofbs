...
public interface DishRepository extends JpaRepository<Dish, Long>{
	@Query(value = "call sp_getDishesByComboId (?1)", nativeQuery = true)
	List<IDishDTO> getDishesByComboId(long comboId);
	
	@Query(value = "call sp_getDishesByRestaurantId (?1, ?2, ?3, ?4)", nativeQuery = true)
	List<IDishDTO> getDishesByRestaurantId(long restaurantId, long categoryId, String dishName, long statusId);
	
//	@Query(value = "call sp_searchDishesByName ?1, ?2", nativeQuery = true)

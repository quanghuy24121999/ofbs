...
public interface OrderRepository extends JpaRepository<Order, Long>{
	@Query(value = "call sp_getOrderByCustomerId (?1, ?2)", nativeQuery = true)
	List<IOrderDTO> getOrderByCustomerId (long customerId, long statusId);
	
	@Query(value = "call sp_getOrderByRestaurantId (?1, ?2)", nativeQuery = true)
	List<IOrderDTO> getOrderByRestaurantId (long restaurantId, String orderCode);
	
	@Modifying
	@Query(value = "call sp_insertOrder (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)", nativeQuery = true)
	...
	@Modifying
	@Query(value = "call sp_setOrderStatus (?1, ?2)", nativeQuery = true)
	void setOrderStatus(long customerId, long restaurantId);
	
	@Modifying
	@Query(value = "call sp_updateOrderStatus (?1, ?2)", nativeQuery = true)
	void updateOrderStatus(long orderId, String status);
	
	@Query(value = "call sp_getTotalOrderByStatus (?1, ?2, ?3)", nativeQuery = true)
	int getTotalOrderByStatus(String status, String fromDate, String toDate);
	
	@Query(value = "call sp_adminGetOrders (?1, ?2, ?3, ?4)", nativeQuery = true)
